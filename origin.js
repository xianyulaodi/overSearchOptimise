		...
		var tmpl = [
			'<div class="blockCont">',
				...
				'<div class="serverTips" style="display:none" id="not_allow_buy_tips">',
					'您所选择的服务器暂不可购买此商品。建议您返回选择服务器，以便筛选到可交易的商品。',
				'</div>',
			'</div>'
		].join('');
		this.dialog = new PopupDialog("选择服务器", tmpl);
...
/**
 * @args_config  每个单独js写的配置文件
 * @query  后端返回字段，跟配置文件对比，如果配置文件在query字段，获取对应的信息
 * @showCount 需要显示的数量
 */
function get_saved_query_title(args_config, query, showCount) {
	...
	for(var i=0;count>0 && i<args_config.length;i++){
		...
		// 这些是区间的字段，后端返回的字段跟这里不一致，所以，需要另外这些值加min,max后缀来判断
		var passArr = ['price','level','sum_exp','equip_level','lingshi_max_level','lingshi_min_level','lingshi_min_duanzao_level','lingshi_max_duanzao_level']
		...
		switch(type) {
			case 'int' :
				if($(id).getChildren('option').length > 0) {
					...
					titleObj[config[1]] = '' + $(id).getChildren('option[value="'+query[key]+'"]')[0].get('text') + ' ';
				}else{
			...
			case 'sub_list' :
				...
				sub_obj =   solve_sub_list(sub_obj,config,query,key,count,showCount);  	
				break;		
		}
	...
	if((query['cross_buy_serverid'] || query['server_type']) && OverallSearchType !== 'role') { //所在服务器
...
function solve_sub_list(sub_obj,config,query,key,count,showCount) {
	var belong = config[3].belong,          // 父类名称
		subType = config[3].sub_type || 'input',    //输入框类型，值为radio,input,select等等
		mark = config[3].mark || '≥',     //标记，显示≥，<=或其他
		listTypeName = config[3].typeName || '',   //list属性中，有些页面不是合并显示，还要显示它的子类型名称
		typeName = config[1] || '',    //子类型名称
		value = query[key] || '',     // 记录的属性对应的值
		elem = config[3].elem || '', //对应的dom值
		text = '';
		if(key == 'growth') {
			value = value / 1000;
		}
	switch(subType) {
		case 'select':
			text = typeName +':' + $$('#'+key+' option[value='+ value +']')[0].innerHTML;
		    break;
		case 'checkbox':
			if(value == 1) {
				text = typeName;
		...
		case 'radio':
		    ...
		    break;
		case 'radio-select':
		    // 角色搜索：飞升，劫渡，化圣需要单独处理，因为有radio和select的结合体
			if(OverallSearchType == 'role') {

				if(value == 1 || value == 2) {

					text = $$(elem+'[value='+value+']').getParent()[0].get('text');
		...
		case 'slider' :		
			...
			if(min_key in query && max_key in query) {
				...
				} else if (listTypeName!='') {  // 等级，除了直接 显示值，有些还要显示名称，比如：身上装备：锻造等级：1-2,装备等级：10-20
	...
	if(text!='' && ((key in query ) || (key + '_min' in query || key + '_max' in query) ) ) {
		if(sub_obj[belong]) {
...
function restore_query_form(args_config, saved_query) {
	...
	for(var i=0;i<args_config.length;i++) {
		...
		var passArr = ['price','level','sum_exp','equip_level','lingshi_max_level','lingshi_min_level','lingshi_min_duanzao_level']
		if(passArr.indexOf(key) > -1) {
			var min_key = key + '_min';
			var max_key = key + '_max';	
...
function restore_query_form_sub_list(config,key,saved_query) {
	...
	var elem = config[3].elem || '';
	if(key in saved_query || (key + '_min' in saved_query && key + '_max' in saved_query) ) {
		switch(subType) {
			case 'radio': 
				// 分两种情况，1，为该选择器在自身上，2，该选择器在它老爸那里,先来第二种情况
				var value = saved_query[key];
			...
			case 'radio-select':
				...
				if(OverallSearchType == 'role') {
					...
					if(value.length > 2 || value > 2) {
						...
						$('zhuang_zhi_hua_sheng').set({'checked':true,'value':value});
			...
			case 'button' :
			    ...
			    break;											
			case 'input': 
				if(key == 'ji_yuan_point' || key == 'addon_point') {  //角色搜索优化特殊处理
...
function save_args_in_cookie(args, search_type)
{
	...
	if (save_ck_name){
		...
		var save_data = {};
		Object.merge(save_data, args, {"search_type":search_type});
		var ck_str = JSON.encode(save_data);
		Cookie.write(save_ck_name, ck_str, {"domain":"163.com"});
	}
}
